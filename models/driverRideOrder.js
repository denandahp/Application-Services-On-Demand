...
class DriverRideOrderModel {
    ...
    async incomingorder(kode) {
        ...
        try {
            let order = await pool.query(`SELECT token_customer, token_driver, landmark_pickup, address_pickup, latitude_location_destination, longitude_location_destination, distance, ongkir, kode_promo, total_price_driver FROM ${jridetb} WHERE "kode" = '${kode}'`);
    ...
    async dataorder(kode) {
        ...
        try {
            let order = await pool.query(`SELECT latitude_location_driver_start, longitude_location_driver_start, latitude_location_pickup, longitude_location_pickup, customer_name, phone_customer, latitude_location_destination, longitude_location_destination, note_pickup, kode, ongkir, kode_promo, total_price_driver, metode_pembayaran FROM ${jrideview} WHERE "kode" = '${kode}'`);
